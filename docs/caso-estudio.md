...
* SchoolBook es una empresa que se dedica a 3 principales actividades:
	* Administrar la operación de las bibliotecas de colegios y escuelas con las que la empresa tiene contratos.
...
* La escuela debe solicitar sus servicios a través de un contrato que contiene los siguientes datos: número de contrato, fecha inicio y fin del contrato, nombre de la escuela o institución, clave escolar,  responsable del contrato (generalmente el director de la escuela).
* En el detalle del contrato también se especifica la lista de bibliotecas que serán administradas por  SchoolBook.  Para cada biblioteca se registra la siguiente información:  Nombre, dirección,  ubicación geográfica (latitud, longitud), dirección de su página web, número aproximado de ejemplares que maneja,  número aproximado de usuarios que maneja.
* Con base a estos datos, en el contrato se especifica la cuota mensual que la escuela deberá pagar.   Una  vez que el contrato es firmado, se obtiene una copia en formato PDF el cual se almacena en base de datos.
...
* Se cuenta con un registro único de usuarios. Este registro le permite al usuario poder acceder al acervo de cualquiera de las bibliotecas de su escuela.  Se registra nombre, apellidos, escuela a la que pertenece, email,  al menos un teléfono, número de cuenta o matrícula y foto tamaño infantil la cual es tomada al momento de realizar el registro. 
#### Registro de Libros.
* Se cuenta con un catálogo global de registro de libros.  Se almacenan los siguientes datos:  Título,  ISBN, editorial, lista de autores, edición, año de publicación, número de página 
* Existen 2 formatos para cada libro: físico y digital.  
* Para los libros físicos se almacena el número total de ejemplares, el número total de ejemplares que se encuentran ubicados en bibliotecas, el número total de ejemplares que se encuentran el almacén y el número total de libros que se consideran como perdidos, ya sea por robo, maltrato, o pérdida por alguna razón.
...
* Cada libro digital se almacena en varios archivos. Se almacena el archivo, el rango de páginas que contiene cada archivo, nombre del archivo, formato y tamaño en bytes.
* Algunos libros digitales incluyen material multimedia, archivos de audio y/o archivos de video.  Para cada archivo se registra un número de archivo, su formato y una descripción.
...
#### Control  de libros en bibliotecas
* Cuando un libro es solicitado o asignado a las bibliotecas se debe registrar en base de datos esta asociación,  su número de clasificación, así como la fecha de asignación. Estos datos son independientes al tipo de libro.
* Para el caso de los libros físicos se almacena el número total de ejemplares con los que cuenta la biblioteca. Este número de ejemplar se incluye en una etiqueta junto al número de clasificación que se pega en el lomo del ejemplar.
* Para cada ejemplar se registra el número de ejemplar iniciando en 0001 y la ubicación dentro de la biblioteca. 
...
* Para llevar el control de los libros se ha diseñado una serie de status.

|Status            |Descripción
| ---              | ---------- 
|EN ALMACEN        | El ejemplar se encuentra en almacén
|EN BIBLIOTECA     | El ejemplar se ha distribuido a una biblioteca
|EN LINEA          | EL libro digital está disponible para ser accedido vía internet
| AGOTADO          | No existen ejemplares en almacén
|NO DISPONIBLE     | Aplica para ambos tipos de libro. Por alguna razón el libro no se encuentra disponible, ya sea para préstamos o para ser accedido via internet.
|BAJA DISPONIBILIDAD| Para los libros físicos, este status indica que hay 3 o menos ejemplares en existencia. Este status se emplea para decidir si se otorgan préstamos a domicilio.
...
* Los prestamos aplican a ambos tipos de libros (ejemplares y digitales).
...
Status  |Descripción  
 ---        | --- 
VIGENTE     | El préstamo se ha otorgado y las fechas de entrega de cada uno de sus ejemplares aun no expiran.
...
CON MULTA   | El Usuario aun no ha realizado la entrega del préstamo, tiene al menos un ejemplar con fecha de entrega expirada y han pasado los 2 días que el sistema otorga para regresar los ejemplares. El importe de la multa a pagar se establecerá una vez que el usuario regrese los ejemplares a la biblioteca.
...
* Las bibliotecas pueden solicitar a SchooBook el surtido ya sea de libros existentes o de nuevos libros.  Para ello se requiere registrar una solicitud de surtido.   Se almacena un folio de solicitud, fecha de solicitud,  usuario que realiza el pedido (generalmente el administrador de la biblioteca).
...
* Adicional a los datos que se guardan de cada libro, se guarda su precio el cual es aplicable en cierto periodo de tiempo. 
* Los usuarios que desean comprar libros deben registrar los datos de al menos una tarjeta de crédito (número, tipo, año y mes de expiración, número de seguridad).   Se requiere registrar de forma obligatoria al menos una dirección de entrega (calle, número, código postal , municipio/delegación/alcaldía/ciudad,  estado y país).
* Cabe mencionar que los usuarios registrados que pertenecen a las escuelas mencionadas anteriormente, pueden hacer uso de sus datos para realizar compras en el sitio web.  Solo deberán registrar los datos de su tarjeta y sus direcciones de envío.
* Para usuarios que no pertenecen a las escuelas, estos no podrán acceder a los prestamos  que son ofrecidos por las escuelas.
...
Status | Descripción
---    | ---
REGISTRADA | La orden de compra está registrada pero aun no ha sido pagada.
...
ENVIADA | Se ha generado un paquete y este ha sido enviado para su entrega. Durante este estado, el usuario puede monitorear el lugar y la fecha de llegada en las diferentes escalas que realice el paquete
